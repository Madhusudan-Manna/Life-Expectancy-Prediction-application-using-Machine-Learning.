...
# Page 3: Model Training & Evaluation
    ...
    if 'model_results' in st.session_state:
        ...
        st.dataframe(results_df.style.apply(highlight_best, axis=0), use_container_width=True)

        # Best model info
        best_metrics = st.session_state['model_results'][best_model_name]
        ...
        with col4:
            st.metric("CV R²", f"{best_metrics['CV R²']:.4f}")
...
# Page 5: Model Performance
    ...
    if 'model_results' not in st.session_state:
        st.warning("⚠️ No model performance data available. Please train models first in the 'Model Training & Evaluation' section.")
    else:
        # Load best model predictions
        best_model_name = st.session_state.get('best_model_name', 'Unknown')
